...
template <typename Node, typename Key>
void BPTree<Node, Key>::insertItem(Node data, Key key) {
	if (root == NULL) {
		root = new BPlusNode<Node, Key>;
		root->key[0] = key;
		root->data[0] = data;
		root->leaf = true;
		root->cnt = 1;
		...
		if (current->cnt < max_size) {
			...
			BPlusNode<Node, Key>* new_leaf = new BPlusNode<Node, Key>;
			...
			if (current == root) {
				...
				BPlusNode<Node, Key>* new_root = new BPlusNode<Node, Key>;
...
void BPTree<Node, Key>::insertInternal(Key key, BPlusNode<Node, Key>* current, BPlusNode<Node, Key>* child) {
	...
	if (current->cnt < max_size) {
		...
		BPlusNode<Node, Key>* new_node = new BPlusNode<Node, Key>;
        Key temp_key[max_size + 1];
